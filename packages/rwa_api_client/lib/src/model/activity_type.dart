...
class ActivityType extends EnumClass {
  ...
  /// 订单类：`market` 市价单 · `limit` 限价单 · `tpsl` 止盈止损 · `close` 平仓 · `liquidation` 强平  资金类：`deposit` 入金 · `external` 外部导入 · `bridge` 划转 · `withdraw` 提现 · `claim` Claim 退款 · `funding` 资金费。内部 Session、Preview、Authorization 和临时签名请求 不属于用户活动。`orderSign` 仅用于已持久化、幂等且可恢复的 bStocks 钱包签名动作；提交后同一 Activity 行转为 `market` 或 `limit`。`approval`、`bridgeSign` 仅为旧客户端反序列化兼容值。 
