...
class ActivityType extends EnumClass {
  ...
  /// 订单类：`market` 市价单 · `limit` 限价单 · `tpsl` 止盈止损 · `close` 平仓 · `liquidation` 强平  资金类：`deposit` 入金 · `external` 外部导入 · `bridge` 划转 · `withdraw` 提现 · `claim` Claim 退款 · `funding` 资金费  签名类：`approval` Token 授权 · `orderSign` 订单签名 · `bridgeSign` 划转签名 
