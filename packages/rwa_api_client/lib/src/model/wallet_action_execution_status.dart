...
class WalletActionExecutionStatus extends EnumClass {
  ...
  static const WalletActionExecutionStatus awaitingUserAuthorization = _$awaitingUserAuthorization;
  /// `submitting` is durably entered with compare-and-set before the synchronous Privy relay. A Privy timeout, 5xx response or process crash after that point is not a deterministic rejection: it transitions to `ambiguous` or `manual_review` and must never be automatically replayed. 
  ...
  static const WalletActionExecutionStatus providerSubmitted = _$providerSubmitted;
  /// `submitting` is durably entered with compare-and-set before the synchronous Privy relay. A Privy timeout, 5xx response or process crash after that point is not a deterministic rejection: it transitions to `ambiguous` or `manual_review` and must never be automatically replayed. 
  ...
  static const WalletActionExecutionStatus userGasConfirmationRequired = _$userGasConfirmationRequired;
  /// `submitting` is durably entered with compare-and-set before the synchronous Privy relay. A Privy timeout, 5xx response or process crash after that point is not a deterministic rejection: it transitions to `ambiguous` or `manual_review` and must never be automatically replayed. 
...
abstract class WalletActionExecutionStatusMixin = Object with _$WalletActionExecutionStatusMixin;
